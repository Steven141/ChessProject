...
impl Moves {
    ...
    pub fn makeMoveCastle(&self, z: &mut Zobrist, mut rook: u64, king: u64, mut hash_key: u64, move_str: &str, p_type: Piece) -> (u64, u64) {
        ...
        if ((king >> start_shift) & 1 == 1) && ((move_str == "0402") || (move_str == "0406") || (move_str == "7472") || (move_str == "7476")) {
    ...
    pub fn makeMoveEP(&self, z: &mut Zobrist, ep: u64, bitboard: u64, mut hash_key: u64, move_str: &str, whites_turn: bool) -> (u64, u64) {
        ...
        if move_str.chars().nth(3).unwrap().is_numeric() {
            ...
            let start_shift: u32 = 64 - 1 - (r1 * 8 + c1);
            if (r1 as i32 - r2 as i32).abs() == 2 && ((bitboard >> start_shift) & 1) == 1 {
    ...
    fn possibleWP(&self, wP: u64, bP: u64, EP: u64) -> String {
        ...
        let mut possible_move: u64 = moves & !wrap_op!(moves, 1, '-'); // selects single possible move
    ...
    fn possibleB(&self, mut B: u64) -> String {
        ...
        let mut bishop: u64 = B & !wrap_op!(B, 1, '-');
        while bishop != 0 {
    ...
    fn possibleQ(&self, mut Q: u64) -> String {
        let mut move_list: String = String::new();
        let mut queen: u64 = Q & !wrap_op!(Q, 1, '-');
        while queen != 0 {
    ...
    fn possibleR(&self, mut R: u64) -> String {
        let mut move_list: String = String::new();
        let mut rook: u64 = R & !wrap_op!(R, 1, '-');
        while rook != 0 {
    ...
    fn possibleN(&self, mut N: u64) -> String {
        let mut move_list: String = String::new();
        let mut knight: u64 = N & !wrap_op!(N, 1, '-');
    ...
    fn possibleK(&self, mut K: u64) -> String {
        let mut move_list: String = String::new();
        let mut king: u64 = K & !wrap_op!(K, 1, '-');
    ...
    pub fn unsafeForBlack(&mut self, mut bitboards: [u64; 13]) -> u64 {
        ...
        // knight threat
        let mut knight: u64 = bitboards[Piece::WN] & !wrap_op!(bitboards[Piece::WN], 1, '-');
        let knight_span_c6_idx: usize = 18;
        while knight != 0 {
            let knight_idx: usize = knight.leading_zeros() as usize;
            // allign the knight_span_c6 mask
        ...
        let mut b_or_q: u64 = wQB & !wrap_op!(wQB, 1, '-');
        while b_or_q != 0 {
        ...
        let mut r_or_q: u64 = wQR & !wrap_op!(wQR, 1, '-');
        while r_or_q != 0 {
        ...
        // king threats
        let mut king: u64 = bitboards[Piece::WK] & !wrap_op!(bitboards[Piece::WK], 1, '-');
        let king_span_c7_idx: usize = 10;
        while king != 0 {
            let king_idx: usize = king.leading_zeros() as usize;
            // allign the king_span_c7 mask
    ...
    pub fn unsafeForWhite(&mut self, mut bitboards: [u64; 13]) -> u64 {
        ...
        unsafe_w |= (bitboards[Piece::BP] >> 9) & !self.masks.file_masks[0]; // pawn left capture

        // knight threat
        let mut knight: u64 = bitboards[Piece::BN] & !wrap_op!(bitboards[Piece::BN], 1, '-');
        let knight_span_c6_idx: usize = 18;
        while knight != 0 {
            let knight_idx: usize = knight.leading_zeros() as usize;
            // allign the knight_span_c6 mask
        ...
        let mut b_or_q: u64 = bQB & !wrap_op!(bQB, 1, '-');
        while b_or_q != 0 {
            let b_or_q_idx: usize = b_or_q.leading_zeros() as usize;
            let moves: u64 = self.possibleDiagAndAntiDiagMoves(b_or_q_idx);
            unsafe_w |= moves;
        ...
        let mut r_or_q: u64 = bQR & !wrap_op!(bQR, 1, '-');
        while r_or_q != 0 {
            let r_or_q_idx: usize = r_or_q.leading_zeros() as usize;
            let moves: u64 = self.possibleHAndVMoves(r_or_q_idx);
            unsafe_w |= moves;
    ...
    pub fn getUpdatedCastleRights(&self, z: &mut Zobrist, move_str: &str, castle_rights: [bool; 4], bitboards: [u64; 13], mut hash_key: u64) -> ([bool; 4], u64) {
        ...
        if move_str.chars().nth(3).unwrap().is_numeric() {
            ...
            if ((1u64 << start_shift) & bitboards[Piece::WK]) != 0 { // white king move
                (castle_rights_t[CastleRights::CWK], castle_rights_t[CastleRights::CWQ]) = (false, false);
            }
            if ((1u64 << start_shift) & bitboards[Piece::BK]) != 0 { // black king move
                (castle_rights_t[CastleRights::CBK], castle_rights_t[CastleRights::CBQ]) = (false, false);
            }
            if ((1u64 << start_shift) & bitboards[Piece::WR] & 1) != 0 { // white king side rook move
                castle_rights_t[CastleRights::CWK] = false;
            }
            if ((1u64 << start_shift) & bitboards[Piece::WR] & (1 << 7)) != 0 { // white queen side rook move
                castle_rights_t[CastleRights::CWQ] = false;
            }
            if ((1u64 << start_shift) & bitboards[Piece::BR] & (1 << 56)) != 0 { // black king side rook move
                castle_rights_t[CastleRights::CBK] = false;
            }
            if ((1u64 << start_shift) & bitboards[Piece::BR] & (1 << 63)) != 0 { // black queen side rook move
                castle_rights_t[CastleRights::CBQ] = false;
            }
            if ((1u64 << end_shift) & 1) != 0 { // white king side rook taken
                castle_rights_t[CastleRights::CWK] = false;
            }
            if ((1u64 << end_shift) & (1 << 7)) != 0 { // white queen side rook taken
                castle_rights_t[CastleRights::CWQ] = false;
            }
            if ((1u64 << end_shift) & ((1 as u64) << 56)) != 0 { // black king side rook taken
                castle_rights_t[CastleRights::CBK] = false;
            }
            if ((1u64 << end_shift) & ((1 as u64) << 63)) != 0 { // black queen side rook taken
                castle_rights_t[CastleRights::CBQ] = false;
            }
        }
