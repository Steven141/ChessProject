...
"""
def findBestMove(game_state, m, z, valid_moves) -> str:
    global next_move, move_counter
    next_move = None
    move_counter = 0
    start_t = time.time()
    findMoveNegaMaxAlphaBeta(game_state, m, z, DEPTH, -CHECKMATE, CHECKMATE, valid_moves)
    return next_move
...
"""
def findMoveNegaMaxAlphaBeta(gs, m, z, depth, alpha, beta, valid_moves) -> None:
    global next_move, move_counter
    bmf = ChessProject.BestMoveFinder(depth)
    bmf.searchPosition(m, z, gs.bitboards, gs.castle_rights, gs.hash_key, gs.whites_turn)
